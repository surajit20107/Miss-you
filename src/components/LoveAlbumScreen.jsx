...
const moments = [
    {
        emoji: "☀️", title: "Lazy Sunday", subtitle: "Just us, all day",
        ...
        color: "from-amber-400/20 to-pink-400/20", border: "border-amber-400/30", tag: "my favorite kind of day 🥹",
    },
    {
        emoji: "🍳", title: "Kitchen Date", subtitle: "Cooking & chaos",
        ...
        color: "from-rose-400/20 to-red-400/20", border: "border-rose-400/30", tag: "better than any restaurant 🍽️",
    },
    {
        emoji: "🌙", title: "Midnight Talks", subtitle: "3am and only you",
        description: "The whole world asleep, but us talking about everything and nothing. Your voice at 3am is my favourite sound in the universe.",
        color: "from-indigo-400/20 to-blue-400/20", border: "border-indigo-400/30", tag: "I never want to hang up 💙",
    },
    {
        emoji: "🎬", title: "Movie Night", subtitle: "I watch you, not the film",
        ...
        color: "from-purple-400/20 to-pink-400/20", border: "border-purple-400/30", tag: "what movie again? 😌",
    },
    {
        emoji: "☕", title: "Morning Coffee", subtitle: "Sleepy eyes & you",
        ...
        color: "from-yellow-400/20 to-orange-400/20", border: "border-yellow-400/30", tag: "you > coffee. always ☀️",
    },
    {
        emoji: "🌟", title: "Stargazing", subtitle: "Infinite sky, just us",
        ...
        color: "from-cyan-400/20 to-purple-400/20", border: "border-cyan-400/30", tag: "you are my favourite star 🌠",
    },
    {
        emoji: "🚗", title: "Late Night Drive", subtitle: "Windows down, music up",
        ...
        color: "from-teal-400/20 to-cyan-400/20", border: "border-teal-400/30", tag: "don't stop singing 🎵",
    },
    {
        emoji: "🌧️", title: "Rainy Day", subtitle: "Rain & you = perfect",
        description: "Rain on the window, a blanket around us, tea going cold. Every rainy day is beautiful because you're in it.",
        color: "from-slate-400/20 to-indigo-400/20", border: "border-slate-400/30", tag: "I love rainy days now 🌧️",
    },
]
...
function buildRows(list) {
    ...
    while (i < list.length) {
        const p = list[i]
        if (p.layout === "full") {
            rows.push({ type: "full", items: [p] }); i++
        } else if (p.layout === "half") {
            const n = list[i + 1]
            if (n?.layout === "half") { rows.push({ type: "half", items: [p, n] }); i += 2 }
            else { rows.push({ type: "full", items: [p] }); i++ }
        } else {
            const p2 = list[i + 1], p3 = list[i + 2]
            if (p2?.layout === "third" && p3?.layout === "third") { rows.push({ type: "third", items: [p, p2, p3] }); i += 3 }
            else if (p2?.layout === "third") { rows.push({ type: "half", items: [p, p2] }); i += 2 }
            else { rows.push({ type: "full", items: [p] }); i++ }
        }
    }
    return rows
        ...
        >
            {(!loaded || err) && (
                <div className="absolute inset-0 bg-gradient-to-br from-pink-500/10 to-purple-500/10 border border-white/5 flex flex-col items-center justify-center gap-1">
                    ...
                    {err && <span className="text-white/15 text-[10px] font-light">{photo.src.split("/").pop()}</span>}
                </div>
            )}
            {!err && (
                <img
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-2">
                <span className="text-white text-[11px] font-light">{photo.caption}</span>
            </div>
        </motion.div>
    )
}
        ...
        <motion.div {...motionProps} className="min-h-screen flex flex-col relative">
            ...
            <div className="sticky top-0 z-20 bg-black/85 backdrop-blur-md px-4 pt-6 pb-3 border-b border-white/5">
                ...
                >
                    ...
                    <p className="text-gray-500 text-xs font-light mt-0.5">every little moment with you… is everything</p>
                </motion.div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pb-28">
                    ...
                    <div className="flex flex-col gap-0.5">
                        {rows.map((row, ri) => (
                            ...
                            >
                                {row.items.map(p => (
                                    <PhotoTile
                                        key={p.id} photo={p} onClick={setLightbox}
                ...
                <div className="flex items-center gap-3 px-4 mt-8 mb-4">
                    ...
                    <span className="text-gray-600 text-xs font-light tracking-widest uppercase">Our Moments</span>
                    <div className="flex-1 h-px bg-white/5" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 px-3 pb-2">
                    {moments.map((m, i) => (
                        <motion.div
                            ...
                            transition={{ delay: 0.07 * i + 0.5, type: "spring", stiffness: 120 }}
                        ...
                        >
                            ...
                            <p className="text-white/40 text-[11px] mb-2 font-light">{m.subtitle}</p>
                            ...
                            <AnimatePresence>
                                {openCard === i && (
                                    ...
                                    >
                                        <p className="text-white/75 text-[11px] leading-relaxed font-light mb-2">{m.description}</p>
                                    </motion.div>
                                )}
                            </AnimatePresence>

                            <p className="text-white/30 text-[10px] italic font-light">{openCard === i ? m.tag : "tap to read ✦"}</p>
                ...
                {lightbox && (
                    ...
                    >
                        ...
                        >
                            ...
                            />
                            <p className="text-white text-center mt-3 text-sm font-light">{lightbox.caption}</p>
                            <div className="flex justify-between mt-4">
                                <button
                                    className="text-white/50 hover:text-white px-4 py-2 text-sm transition-colors"
                                    onClick={() => { const idx = photos.findIndex(p => p.id === lightbox.id); setLightbox(photos[(idx - 1 + photos.length) % photos.length]) }}
                                >← Prev</button>
                                <button
                                ...
                                <button
                                    className="text-white/50 hover:text-white px-4 py-2 text-sm transition-colors"
                                    onClick={() => { const idx = photos.findIndex(p => p.id === lightbox.id); setLightbox(photos[(idx + 1) % photos.length]) }}
                                >Next →</button>
                            </div>
                        </motion.div>
                    </motion.div>
